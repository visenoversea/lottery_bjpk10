...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getCountryAreaList(query)
  table.loading = false
  if (!success) return
  ...
  <el-card>
    ...
    <el-form :inline="true">
      ...
      <el-form-item label="任务名字">
        <el-input v-model="query.country"  @keyup.enter="getList" placeholder="请输入任务名字" @clear="getList" clearable />
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="table.loading" :data="table.list" stripe border>
      ...
      <el-table-column label="国家" min-width="160">
        <template #default="scope">
          <span class="g-purple">{{scope.row.country}}</span>
        </template>
      </el-table-column>
      <el-table-column label="区号" min-width="130">
        <template #default="scope">
          <span class="g-red">{{scope.row.area_code}}</span>
        </template>
      </el-table-column>
