...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getUserRebateList(query)
  table.loading = false
  if (!success) return
  ...
  <el-card>
    ...
    <el-table v-loading="table.loading" :data="table.list" stripe border>
      ...
      <el-table-column label="类型" width="80">
        <template #default="scope">
          <span v-if="scope.row.type==0" class="g-blue-tiffany">其他</span>
          <span v-else-if="scope.row.type==1" class="g-green">充值</span>
          <span v-else-if="scope.row.type==2" class="g-blue">量化</span>
          <span v-else class="g-red">异常</span>
        </template>
      </el-table-column>
      <el-table-column prop="currency" label="货币" width="80">
      </el-table-column>
      <el-table-column label="返佣金额" min-width="100">
        <template #default="scope">
          <span :class="[scope.row.amount>=0?'g-red':'g-green']">{{ scope.row.amount }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="rebate.user_name" label="返佣人" min-width="80">
      </el-table-column>
