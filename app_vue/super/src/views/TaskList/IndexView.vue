...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getTaskList(query)
  table.loading = false
  if (!success) return
  ...
  <el-card>
    ...
    <el-form :inline="true">
      ...
      <el-form-item label="任务名字">
        <el-input v-model="query.title"  @keyup.enter="getList" placeholder="请输入任务名字" @clear="getList" clearable />
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="table.loading" :data="table.list" stripe border>
      ...
      <el-table-column label="任务名字" min-width="100">
        <template #default="scope">
          <span class="g-purple">{{scope.row.title}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="任务奖励" min-width="160">
        <template #default="scope">
          <span class="g-red">{{scope.row.amount}}</span>
        </template>
      </el-table-column>
