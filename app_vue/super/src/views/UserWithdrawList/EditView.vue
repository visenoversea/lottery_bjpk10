...
const confirm = async () => {
  ...
  loading.value = true
  const {success, data} = await api.editUserWithdraw(form)
  loading.value = false
  if (!success) return
  emits('success')
...
<!--        </el-col>-->
      ...
      <el-row :gutter="24">
        <el-col :span="12">
          <el-form-item label="到账金额">
            <el-input v-model="form.real_amount" placeholder="请输入实际到账金额"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-row :gutter="24">
        <el-col :span="12">
          <el-form-item class="g-red" label="提现金额"><span>{{ props.data.amount }}</span></el-form-item>
<!--          <el-form-item class="g-red" label="提现金额"><span>{{ props.data.amount }} USDT</span><span v-if="form.info.currency!='USDT'">&nbsp;≈&nbsp;{{ Number(props.data.amount*form.info.rate).toFixed(form.info.fixed) }} {{form.info.currency}}</span></el-form-item>-->
        </el-col>
        <el-col :span="12">
          <el-form-item class="g-purple" label="手续费">
      ...
      <el-form-item v-if="props.data.status==0" label="失败原因" prop="name">
