...
const editStatus = (row,status) => {
  if (status === 1) {
    ElMessageBox.confirm('确认提现审核通过?', '提示', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'})
        .then(async () => {
          table.loading = true
          const {success, data} = await api.reviewUserWithdraw({id: row.id, status: 1})
          table.loading = false
          if (!success) return
    ...
    ElMessageBox.prompt('审核不通过原因(选填)', '确认驳回提现审核', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'})
        .then(async ({value}) => {
          table.loading = true
          const {success, data} = await api.reviewUserWithdraw({id: row.id, reason: value, status: 0})
          table.loading = false
          if (!success) return
          ElMessage.success(data.msg)
          await getList(false)
...
<template>
  <el-card>
    ...
    <div class="g-grey g-p5">筛选结果统计:真实提现订单总金额：<span class="g-red">{{table.totalAmount}}</span>, 真实待审核提现订单总金额：<span class="g-red">{{table.reviewAmount}}</span></div>
    <el-form :inline="true">
...
<!--        </template>-->
<!--      </el-table-column>-->
      <el-table-column prop="nums" label="提现次数" width="75" :show-overflow-tooltip="true">
      </el-table-column>
...
<!--      </el-table-column>-->
      ...
      <el-table-column label="状态" width="80" >
        <template #default="scope">
          ...
          <span v-else-if="scope.row.status===3" class="g-purple">提款中</span>
          <span v-else-if="scope.row.status===0" class="g-red">审核不通过</span>
          <span class="g-red" v-else>异常</span>
        </template>
      </el-table-column>
      <el-table-column label="提现前余额" min-width="150" :show-overflow-tooltip="true">
        <template #default="scope">
          <div class="g-red">{{ scope.row.beforeBalance }}</div>
        </template>
      </el-table-column>
      <el-table-column label="申请金额" min-width="150" :show-overflow-tooltip="true">
        <template #default="scope">
...
<!--          <div class="g-green" v-if="scope.row.currency!=scope.row.info.currency">≈ {{ (scope.row.apply_amount*scope.row.info.rate).toFixed(scope.row.info.fixed) }} <span class="g-blue">{{scope.row.info.currency}}</span></div>-->
      ...
      <el-table-column label="实际金额" min-width="150" :show-overflow-tooltip="true">
        <template #default="scope">
...
<!--          <div class="g-purple">{{ scope.row.fee }} <span class="g-blue">{{scope.row.currency}}</span></div>-->
      ...
      <el-table-column label="到账金额" min-width="150" :show-overflow-tooltip="true">
        <template #default="scope">
...
<!--          <div class="g-green" v-if="scope.row.currency!=scope.row.info.currency">≈ {{ (scope.row.real_amount*scope.row.info.rate).toFixed(scope.row.info.fixed) }} <span class="g-blue">{{scope.row.info.currency}}</span></div>-->
      ...
      <el-table-column label="提现信息" min-width="160" :show-overflow-tooltip="true">
        <template #default="scope">
...
<!--      </el-table-column>-->
      ...
      <el-table-column v-if="auth.editUserWithdraw" label="操作" width="130" fixed="right">
        <template #default="scope">
          <div style="padding-bottom: 12px" v-if="auth.reviewUserWithdraw && scope.row.status===2">
          ...
          <el-button v-if="scope.row.status===2 && auth.DPayWithdraw" type="primary" @click="DPay(scope.row)">DPay</el-button>
          <el-button type="primary" @click="edit(scope.row)">编辑</el-button>
          <el-button v-if="auth.delUserWithdraw" type="danger" @click="del(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
