...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getMiningList(query)
  table.loading = false
  if (!success) return
...
const del = (row) => {
  ElMessageBox.confirm('确认删除数据?', '提示',
    ...
    table.loading = true
    const {success, data} = await api.delMining({id: row.id})
    table.loading = false
    if (!success) return
...
<template>
  <el-card>
    ...
    <el-table v-loading="table.loading" :data="table.list" stripe border>
      ...
      <el-table-column prop="day" label="周期" width="80" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column label="日收益" min-width="120" :show-overflow-tooltip="true">
        <template #default="scope">
          <span>{{scope.row.min_rate*100}}-{{scope.row.max_rate*100}}%</span>
        </template>
      </el-table-column>
      <el-table-column label="违约比例" width="80" :show-overflow-tooltip="true">
        <template #default="scope">
          <span>{{scope.row.bc_rate*100}}%</span>
        </template>
      </el-table-column>
      <el-table-column prop="min" label="最小" width="60" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="max" label="最大" width="60" :show-overflow-tooltip="true">
        <template #default="scope">
