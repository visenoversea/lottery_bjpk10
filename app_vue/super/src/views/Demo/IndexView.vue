...
// const end_time = new Date(start_time.setHours(0, 0, 0, 0) + 86399000)
...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getDemoList(query)
  table.loading = false
  if (!success) return
...
const del = (row) => {
  ElMessageBox.confirm('确认删除数据?', '提示',
    ...
    table.loading = true
    const {success, data} = await api.delDemo({id: row.id})
    table.loading = false
    if (!success) return
...
<template>
  <el-card>
    ...
    <el-table v-loading="table.loading" :data="table.list" @selection-change="selectionChange" stripe border>
