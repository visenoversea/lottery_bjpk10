...
const auth = reactive({
  editUserWallet:store.auth('editUserWallet'),
})
//表单
...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getUserWalletList(query)
  table.loading = false
  if (!success) return
...
const del = (row) => {
  ElMessageBox.confirm('确认删除数据?', '提示',
    ...
    table.loading = true
    const {success, data} = await api.delUserWallet({id: row.id})
    table.loading = false
    if (!success) return
  ...
  <el-card>
    ...
    <el-form :inline="true">
      ...
      <el-form-item label="币种">
        <el-input v-model="query.currency"  @keyup.enter="getList" placeholder="请输入币种" @clear="getList" clearable />
      </el-form-item>
      <el-form-item>
        <template #label>
    ...
    <el-table v-loading="table.loading" :data="table.list" stripe border>
      ...
      <el-table-column prop="currency" label="币种" width="80" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="chain" label="链类型" min-width="80">
      </el-table-column>
      ...
      <el-table-column v-if="auth.editUserWallet" label="操作" width="130" fixed="right">
        <template #default="scope">
