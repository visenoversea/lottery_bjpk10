...
const auth = reactive({
  editUserBank:store.auth('editUserBank'),
  delUserBank:store.auth('delUserBank')
})
//表单
...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getUserBankList(query)
  table.loading = false
  if (!success) return
  ...
  <el-card>
    ...
    <el-form :inline="true">
      ...
      <el-form-item label="银行名称">
        <el-input v-model="query.bank_name"  @keyup.enter="getList" placeholder="请输入银行名称" @clear="getList" clearable />
      </el-form-item>
    ...
    <el-table v-loading="table.loading" :data="table.list" stripe border>
      ...
      <el-table-column prop="bank_name" label="银行名称" min-width="80" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="bank_code" label="银行代码" min-width="80" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="branch" label="开户支行" min-width="80" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="name" label="姓名" min-width="80" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="card_number" label="卡号" min-width="100" :show-overflow-tooltip="true">
      </el-table-column>
      ...
      <el-table-column v-if="auth.editUserBank" label="操作" width="130" fixed="right">
        <template #default="scope">
          <el-button type="primary" @click="edit(scope.row)">编辑</el-button>
          <el-button v-if="auth.delUserBank" type="danger" @click="del(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
