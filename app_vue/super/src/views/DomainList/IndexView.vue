...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getDomainList(query)
  table.loading = false
  if (!success) return
...
const del = (row) => {
  ElMessageBox.confirm('确认删除数据?', '提示',
    ...
    table.loading = true
    const {success, data} = await api.delDomain({id: row.id})
    table.loading = false
    if (!success) return
  ...
  <el-card>
    ...
    <el-form :inline="true">
      ...
      <el-form-item label="主机">
        <el-row>
          <el-col :span="18">
            <el-input v-model="query.host"  @keyup.enter="getList" placeholder="请输入主机" @clear="getList" clearable />
          </el-col>
    ...
    <el-table v-loading="table.loading" :data="table.list" stripe border>
      ...
      <el-table-column label="用户" width="120">
        <template #default="scope">
          <div>
            <span v-if="scope.row.type===0" class="g-red">前端域名</span>
            <span v-else-if="scope.row.type===1" class="g-blue">APP域名</span>
            <span v-else-if="scope.row.type===2" class="g-purple">后台域名</span>
            <span v-else-if="scope.row.type===3" class="g-green">下载域名</span>
            <span v-else class="g-red">(异常)</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="host" label="主机" min-width="80" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="port" label="端口" min-width="80" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="sort" label="排序" min-width="80" :show-overflow-tooltip="true">
      </el-table-column>
