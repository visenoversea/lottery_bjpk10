...
const confirm = async () => {
  ...
  loading.value = true
  const {success, data} = await api.editLottery(form)
  loading.value = false
  if (!success) return
  emits('success')
...
<template>
  <el-dialog v-model="show" title="编辑" @open="open" draggable :close-on-click-modal="false" width="580px">
    <el-form v-loading="loading" size="default" :model="form" label-width="90px">
      <el-form-item v-if="props.data.lottery" label="彩种">{{props.data.name}}</el-form-item>
      <el-form-item label="杀率">
        <el-input v-model="form.risk" placeholder="请输入杀率0-100"></el-input>
      </el-form-item>
