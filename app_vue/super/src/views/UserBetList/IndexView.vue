...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getUserBetList(query)
  table.loading = false
  if (!success) return
...
<!--      </el-table-column>-->
      ...
      <el-table-column prop="open_expect" label="期号" min-width="110">
      </el-table-column>
      <el-table-column label="状态" width="80" >
        <template #default="scope">
          <span v-if="scope.row.status===1" class="g-green">已开奖</span>
          <span v-else-if="scope.row.status===2" class="g-red">待开奖</span>
          <span v-else-if="scope.row.status===0" class="g-purple">撤单</span>
          <span class="g-red" v-else>异常</span>
        </template>
      </el-table-column>
      <el-table-column prop="open_code" label="开奖结果" min-width="110">
      </el-table-column>
      <el-table-column label="投注总金额" min-width="90" :show-overflow-tooltip="true">
        <template #default="scope">
      ...
      <el-table-column label="操作" width="80" fixed="right">
        <template #default="scope">
          <el-button type="info" @click="lookItem(scope.row)">详情</el-button>
<!--          <el-button type="danger" @click="del(scope.row)">删除</el-button>-->
    ...
    />
    <ItemList v-model="itemShow" :data="table.row"/>
  </el-card>
</template>