...
const confirm = async () => {
  ...
  loading.value = true
  const {success, data} = await api.addProduct(form)
  loading.value = false
  if (!success) return
  emits('success')
...
<template>
  <el-dialog v-model="show" top="2vh" title="新增" @open="open" @close="close" :close-on-click-modal="false"
             width="750px">
...
<!--              <el-option label="币安" :value="3"></el-option>-->
        ...
        <el-col :span="12">
          <el-form-item v-if="form.api!=1" label="api参数" prop="api_config.symbol">
            <el-input v-model="form.api_config.symbol" placeholder="例btcusdt,ethusdt" autocomplete="off"></el-input>
          </el-form-item>
        </el-col>
