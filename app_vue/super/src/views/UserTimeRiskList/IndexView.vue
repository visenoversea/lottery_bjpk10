...
const getList = async (init = true) => {
  ...
  table.loading = true
  const {success, data} = await api.getUserTimeRiskList(query)
  table.loading = false
  if (!success) return
...
const del = (row) => {
  ElMessageBox.confirm('确认删除数据?', '提示',
    ...
    table.loading = true
    const {success, data} = await api.delUserTimeRisk({id: row.id})
    table.loading = false
    if (!success) return
...
<template>
  <el-card>
    ...
    <el-table v-loading="table.loading" :data="table.list" stripe border>
      ...
      <el-table-column prop="risk" label="胜率" min-width="100" :show-overflow-tooltip="true">
      </el-table-column>
