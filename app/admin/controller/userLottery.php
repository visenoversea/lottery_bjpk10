...
class userLottery extends base
{
    ...
    public function getList($userType = '', $virtual = '', $status = '', $search_key = 'user_id', $search_val = '', $page = 1, $limit = 15)
    {
        ...
        $data = $user_lottery_model->with(['user' => 'id,type,pid,layer,user_name,virtual,status'])
            ->where($where)
    ...
    public function add($user_id, $nums = 1, $lottery_id = 0)
    {
        ...
        if ($data['lottery_id'] > 0) {
            ...
            $lottery = $lottery_model->where(['id' => $data['lottery_id']])->getOne();
            if (!$lottery) {
    ...
    public function edit($id, $lottery_id = 0)
    {
        ...
        $user_lottery_model = user_lottery_model::getInstance();
        $userLottery = $user_lottery_model->where(['id' => $data['id']])->getOne();
        if (!$userLottery) {
            $this->GlobalService->json(['code' => -2, 'msg' => '未知的ID']);
        }
        if ($userLottery['status'] != 2) {
    ...
    {
        $user_lottery_model = user_lottery_model::getInstance();
        $res = $user_lottery_model->delById($id);
