...
class userProductContract extends base
{
    public function getList($userType = '', $virtual = '', $side = '', $order_no = '', $status = '', $date_time = [], $search_key = 'user_id', $search_val = '', $page = 1, $limit = 15)
    {
        ...
        $data = $user_product_contract_model
    ...
    public function sell($id)
    {
        ...
        if ($res['code'] == 1) {
            /*********添加日志*********/
            (user_log_model::getInstance())->addLog('合约订单平仓', $userProductContract);
            /*********添加日志*********/
        }
    ...
    {
        $user_product_contract_model = user_product_contract_model::getInstance();
        $userProductContract = $user_product_contract_model->where(['id' => $id,  'del' => 0])->getOne();
        if (!$userProductContract) {
        ...
        if ($res['code'] == 1) {
            /*********添加日志*********/
            (user_log_model::getInstance())->addLog('合约委托订单取消', $userProductContract);
            /*********添加日志*********/
        }
        $this->GlobalService->json($res);
    }
    ...
    {
        $user_product_contract_model = user_product_contract_model::getInstance();
        $res = $user_product_contract_model->delById($id);
        $this->GlobalService->json($res);
    }
